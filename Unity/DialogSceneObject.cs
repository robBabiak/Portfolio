 ...
 *
 *
 * This code is used for dialog scene objects.
...
[Serializable]
public partial class DialogSceneObject : InteractiveObject
{
    public int dialogID = 0;
    private DialogManager dialog = null;
    public DialogScene scene = new DialogScene();
    ...
    {
		// if log is enabled, display log
        if (log) Debug.Log("Activate Dialog " + dialogID);
        if (dialogID != 0)
        {
            return dialog.BeginDialog(dialogID, scene); // start dialog
        }
        return false;
    }
}
